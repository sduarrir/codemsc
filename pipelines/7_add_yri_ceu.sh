...
plink --bfile mergeLAZ_FM_PJLYRICEU_comsnp --recode vcf --out mergeLAZ_FM_PJLYRICEU_comsnp
module load Java/12.0.2
sbatch --job-name=ibd --nodes=1 --ntasks-per-node=1 --output=ibd_wds.out --wrap="module load Java/12.0.2; java -Xmx8g -jar ../subsetspn/ibd/ibdseq.r1206.jar gt=mergeLAZ_FM_PJLYRICEU_comsnp.vcf out=idb_wds"
